...
MatrixXd UKF::generate_aug_sigma_points(){
  // cout<<"**generate_aug_sigma_points"<<endl;
...
void UKF::predict_sigma_points(const MatrixXd &Xsig_aug, double delta_t){
  // cout<<"**predict_sigma_points"<<endl;
...
void angle_normalization(MatrixXd &a, int i){
  double angle;
...
void UKF::predict_mean_covariance(){
  // cout<<"**predict_mean_covariance"<<endl;
...
void UKF::Prediction(double delta_t) {
  ...
  MatrixXd Xsig_aug = generate_aug_sigma_points();
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  NIS_laser_ = predict_measurement(meas_package.raw_measurements_, Zsig, R_laser, -1, -1);
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  NIS_radar_ = predict_measurement(meas_package.raw_measurements_, Zsig, R_radar, 1, 3);
  cout<<"NIS_radar_ = "<<NIS_radar_<<endl;
}

double UKF::predict_measurement(const VectorXd &z, const MatrixXd &Zsig, const MatrixXd &R, int zangle_idx, int xangle_idx){
  //mean predicted measurement
  VectorXd z_pred = Zsig*weights_;
  MatrixXd zsig_diff = Zsig.colwise() - z_pred;
  MatrixXd x_diff = Xsig_pred_.colwise() - x_;

  if(zangle_idx >= 0) angle_normalization(zsig_diff, zangle_idx);
  if(xangle_idx >= 0) angle_normalization(x_diff, xangle_idx);
